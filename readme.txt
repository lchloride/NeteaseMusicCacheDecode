...
·图形界面只适用于Windows系统，*nix系统需要自行编译源码。
...
2. 输出目录一项需要填入希望输出到的文件夹；如果指定“使用缓存文件目录”，则该项不需要填写。
   输出目录格式如：F:\Music，注意末尾不需要反斜杠(\)。

3. 输出文件名一项需要填入希望得到的音乐文件名，文件名不需要扩展名，默认为mp3；如果指定“使用输入文件名”，则该项不需要填写。
...
-------------------------------------
声明
本程序为免费程序，分享请注明出处，尊重开发者版权。本程序传播过程中造成的任何形式的后果与开发者无关。
